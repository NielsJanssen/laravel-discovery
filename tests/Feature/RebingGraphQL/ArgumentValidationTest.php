 ...
 */
describe('validation attributes on action parameters', function () {
...
describe('error keys under a renamed arg', function () {
    ...
    it('keys #[Each] element errors under the arg name, never the PHP parameter name', function () {
        // notifyAll's parameter is $notify but the arg is `recipients`: every key of the payload,
        // element index included, has to be translated back to the arg the client actually sent.
        $response = $this->postJson('/graphql', [
            'query' => '{ notifyAll(recipients: ["nope", "also-nope"]) }',
        ])->assertOk();

        $validation = $response->json('errors.0.extensions.validation');

        expect($validation)->toHaveKey('recipients.0');

        expect($validation)->toHaveKey('recipients.1');

        expect(array_keys($validation))->not->toContain('notify.0');

        expect(array_keys($validation))->not->toContain('notify.1');
    });
});
